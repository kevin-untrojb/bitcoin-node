...
use crate::log::{log_info_message,log_error_message,LogMessages};
    ...
    match bytes_read_option {
        Some(read_bytes) => {
            if read_bytes > 0 {
                ...
                return read_bytes_header(logger.clone(),&connection, admin_connections, intento + 1);
            };
            log_error_message(logger.clone(),format!("Máximo de reintentos alcanzado ... probá más tarde ..."));
...
fn get_headers_filtrados(logger:Sender<LogMessages>,blockheaders: &Vec<BlockHeader>) -> Vec<BlockHeader> {
    ...
    log_info_message(logger.clone(),format!(
    ...
    match admin_connections.lock() {
        Ok(mut admin) => {
            let (thread_connection, thread_id_connection) = match admin.find_free_connection() {
                ...
                Err(_) => {
                    log_error_message(logger.clone(),format!("Error al buscar conexiones libres."));
...
fn change_mutex_connection_id(
    ...
    if intento > total_reintentos() {
        log_info_message(logger.clone(),format!("Máximo de reintentos alcanzado ... probá más tarde ..."));
        return Err(NodoBitcoinError::NoSeEncuentraConexionLibre);
    }
...
fn read_bytes_data(logger:Sender<LogMessages>,connection: &Connection) -> Result<[u8; 24], NodoBitcoinError> {
    ...
    match thread_bytes_read_result {
        ...
        Err(_) => {
            log_error_message(logger.clone(),format!("Error al leer mensaje {:?}", thread_buffer));
            Err(NodoBitcoinError::NoSePuedeLeerLosBytes)
        }
    }
}
...
fn write_bytes_data(
    ...
    if writed_connection.is_err() {
        log_error_message(logger.clone(),format!("Error al escribir el mensaje get_data"));
        (connection, _) =
            change_mutex_connection_id(logger.clone(),connection_id, admin_connections, intento + 1)?;
...
fn thread_data(
    ...
    let (mut cloned_connection, mut thread_id_connection) =
        unwrap_or_return!(get_mutex_connection_id(logger.clone(),&admin_connections_mutex_thread));
        ...
        loop {
            ...
            if valid_command {
                ...
                if cloned_result.is_err() {
                    log_info_message(logger.clone(),format!("Error al lockear el vector de bloques"));
                    return;
                }
                ...
                let block = match SerializedBlock::deserialize(&response_get_data) {
                    Ok(block) => block,
                    Err(_) => {
                        log_error_message(logger.clone(),format!("Error al deserializar el bloque {:?}", response_get_data));
                        continue;
                    }
                };
                cloned.push(block);
    ...
    liberar_conexion(logger.clone(),thread_id_connection, admin_connections_mutex_thread);
}

pub fn get_full_blockchain(logger:Sender<LogMessages>,admin_connections: AdminConnections) -> Result<(), NodoBitcoinError> {
    ...
    loop {
        ...
        if valid_command {
            ...
            // guardar bloques
            blocks_joined_guardar(logger.clone(),&blocks, headers_filtrados_len, blockheaders, reintentos)?;
            reintentos += 1;
...
fn blocks_joined_guardar(
    ...
    if !bloques_a_guardar.is_empty() {
        log_info_message(logger.clone(),format!("Bloques descargados: {:?}", bloques_a_guardar.len()));
    };
    ...
    if bloques_a_guardar.len() == headers_filtrados_len {
        ...
        log_info_message(logger.clone(),format!("No se descargaron todos los bloques, reintentando ..."));
    }
    Ok(())
}
